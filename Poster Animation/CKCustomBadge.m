...
- (void) autoBadgeSizeWithString:(NSString *)badgeString
{
    ...
    CGSize stringSize = [[UIFactory sharedInstance] sizeWithMyFont:[UIFont boldSystemFontOfSize:12] withText:badgeString withWidth:25.0];
...
+ (CKCustomBadge*) customBadgeWithString:(NSString *)badgeString
{
...
+ (CKCustomBadge*) customBadgeWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining
{
...
- (void)drawRect:(CGRect)rect
{
	...
	if ([self.badgeText length] > 0 )
    {
        ...
        CGSize textSize = [[UIFactory sharedInstance] sizeWithMyFont:textFont withText:self.badgeText withWidth:rect.size.width];
        NSMutableAttributedString *title;
        title = [[NSMutableAttributedString alloc] initWithString:self.badgeText];
        int length = (int)title.length;
        [title addAttribute:NSFontAttributeName value:[UIFont boldSystemFontOfSize:sizeOfFont] range:NSMakeRange(0,length)];
        [title addAttribute:NSForegroundColorAttributeName value:self.badgeTextColor range:NSMakeRange(0,length)];
        [title drawAtPoint:CGPointMake((rect.size.width/2-textSize.width/2), (rect.size.height/2-textSize.height/2))];
	}
