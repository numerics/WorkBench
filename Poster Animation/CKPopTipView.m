...
- (CGRect)contentFrame
{
	...
	CGRect contentFrame = CGRectMake(bubbleFrame.origin.x + _cornerRadius, bubbleFrame.origin.y + _cornerRadius,
									 (bubbleFrame.size.width - _cornerRadius*2), (bubbleFrame.size.height - _cornerRadius*2));
	return contentFrame;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.title)
    {
        ...
        //[self.title drawInRect:titleFrame withFont:self.titleFont lineBreakMode:NSLineBreakByClipping alignment:self.titleAlignment];
        
        [self.title drawInRect:titleFrame withAttributes:@{NSFontAttributeName:self.titleFont,NSParagraphStyleAttributeName:textStyle}];
    }
	...
	if (self.message)
    {
        ...
        if (self.title)
        {
            CGSize tSize = [[UIFactory sharedInstance] sizeWithMyFont:self.titleFont withText:self.title withWidth:textFrame.size.width lineBreakMode:NSLineBreakByClipping];
            textFrame.origin.y += tSize.height;
            
            //textFrame.origin.y += [self.title sizeWithFont:self.titleFont constrainedToSize:CGSizeMake(textFrame.size.width, 99999.0) lineBreakMode:NSLineBreakByClipping].height;
        }
        ...
        textStyle.alignment = self.titleAlignment;
        [self.message drawInRect:textFrame withAttributes:@{NSFontAttributeName:self.textFont,NSParagraphStyleAttributeName:textStyle}];

        //[self.message drawInRect:textFrame withFont:self.textFont lineBreakMode:NSLineBreakByWordWrapping alignment:self.textAlignment];
    }
}
...
- (void)presentPointingAtView:(UIView *)targetView inView:(UIView *)containerView animated:(BOOL)animated
{
    ...
    if (self.message!=nil)
    {
        textSize = [[UIFactory sharedInstance] sizeWithMyFont:self.textFont withText:self.message withWidth:rectWidth lineBreakMode:NSLineBreakByWordWrapping];
        //textSize= [self.message sizeWithFont:self.textFont constrainedToSize:CGSizeMake(rectWidth, 99999.0) lineBreakMode:NSLineBreakByWordWrapping];
    }
    ...
    if (self.title != nil)
    {
        textSize = [[UIFactory sharedInstance] sizeWithMyFont:self.titleFont withText:self.title withWidth:rectWidth lineBreakMode:NSLineBreakByClipping];
...
{
	__strong id<CKPopTipViewDelegate> delegate = self.delegate;
