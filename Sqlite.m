...
@synthesize filePath,selectionOption;
...
- (NSMutableArray *)executeQuery:(NSString *)sql arguments:(NSArray *)args
{
	...
	while ([self hasData:sqlStmt])
	{
		...
		{
			[dictionary setObject:[NSNumber numberWithInteger:0] forKey:@"selected"];		//defaults to de-selected state
		}
...
- (NSMutableArray *)executeQueryCol:(NSString *)col  withSql:(NSString *)sql, ...
{
	...
	NSMutableArray *result = [self executeQuery:sql forCol:col arguments:argsArray];
	
	return result;
}

- (NSMutableArray *)executeQuery:(NSString *)sql forCol:(NSString *)col arguments:(NSArray *)args
{
	sqlite3_stmt *sqlStmt;
	...
	for (i = 0; i < columnCount; ++i)
	{
		NSString *columnName = [self columnName:sqlStmt columnIndex:i];
		if([columnName isEqualToString:col] )
		{
			colnm = i;
			break;
		}
	...
	while ([self hasData:sqlStmt])
	{
		id columnData = [self columnData:sqlStmt columnIndex:colnm];
		[arrayList addObject:columnData];
	}
	sqlite3_finalize(sqlStmt);
